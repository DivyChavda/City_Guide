...
<head>
        ...
        <title>City_Guide | ViewPicture</title>
                            ...
                            <div class="col-md-12">
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                        <h3 class="panel-title">View Picture</h3>
                                    </div>
                                            ...
                                            <div class="col-md-12 col-sm-12 col-xs-12">
                                                <table id="datatable" class="table table-striped table-bordered">
                                                    <thead>
                                                        <tr>
                                                            <th>Picture Id</th>
															<th>Picture Images</th>
															<th>Action</th>
                                                        </tr>
                                                    </thead>
													<tbody> 
													<?php
													$res = $con->query("select * from advertisement") or die(mysqli_error($con));
													while($row = mysqli_fetch_array($res))
													{
													?>
                                                        <tr>
                                                            <td><?php echo $row["AdvertisementId"]; ?></td>
															<td><img src="Advertisement_Image/<?php echo $row["AdvertisementImage"]; ?>" height="100" width="100"></td>
															<td><a href="AddPicture.php?update=<?php echo $row["AdvertisementId"]; ?>">Edit</a> | <a href="?delete=<?php echo $row["AdvertisementId"]; ?>">Delete</a></td>
                                                        </tr>
													<?php } ?>
                                                    </tbody>
													...
													<?php
														...
														{
															$result=$con->query("select AdvertisementImage from advertisement where AdvertisementId='".$_REQUEST["delete"]."'");
															while($row1=mysqli_fetch_array($result))
															{
																$image1=$row1["AdvertisementImage"];
															}
															unlink("AdvertisementImage//".$image1);
															$con->query("delete from advertisement where AdvertisementId='".$_REQUEST["delete"]."'") or die(mysqli_error($con));
