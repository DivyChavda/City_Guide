...
<head>
        ...
        <title>CityGuide | ViewCategory</title>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 col-sm-12 col-xs-12">
                                                <table id="datatable" class="table table-striped table-bordered">
													...
													<tbody> 
													<?php
													$res = $con->query("select * from category") or die(mysqli_error($con));
													while($row = mysqli_fetch_array($res))
													{
													?>
                                                        <tr>
                                                            <td><?php echo $row["CategoryName"]; ?></td>
															<td><img src="CategoryIcons/<?php echo $row["CategoryIcon"]; ?>" height="100" width="100"></td>
															<td><a href="AddCategory.php?update=<?php echo $row["CategoryId"]; ?>">Edit</a> | <a href="?delete=<?php echo $row["CategoryId"]; ?>">Delete</a></td>
                                                        </tr>
													<?php } ?>
                                                    </tbody>
													...
													<?php
														...
														{
															$result=$con->query("select CategoryIcon from category where CategoryId='".$_REQUEST["delete"]."'");
															while($row1=mysqli_fetch_array($result))
															{
																$image1=$row1["CategoryIcon"];
															}
															unlink("CategoryIcons//".$image1);
															$con->query("delete from category where CategoryId='".$_REQUEST["delete"]."'") or die(mysqli_error($con));
															echo "<script>alert('Record deleted..!!');window.location='ViewCategory.php'</script>";
														}
													?>
                                            </div>
                                        </div>
